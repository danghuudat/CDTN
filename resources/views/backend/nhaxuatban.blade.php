...
@section('content')
    ...
    <!-- Modal -->
    <div class="modal fade" id="NXBModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog " id="Modalnhaxuatban" role="document">
                ...
                <form id="formsubmit">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label  class="col-form-label">Tên Nhà Xuất Bản:</label>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var table= $('#example').DataTable({
                ...
                ajax:'{{asset('admin/book/nxb/data')}}',
                columns:[
                    {data:'id',"width":"5%"},
                    {data:'name_nxb'},
                    {data:'Modifly',
            ...
            $(document).on('click','.edit',function () {
                ...
                $.ajax({
                    url: '{{asset("admin/book/nxb/edit")}}',
                    type: 'GET',
                    dataType: 'json',
                    data: {id:$(this).val()},
                    success:function(data){
                        $('#NXBModal').modal('show');
                        $('.modal-title').text('Edit '+data.name_nxb);
                        $('.submitbutton').text('Update');
                        $('#action').val('Edit');
                        $('#name').val(data.name_nxb);
                        $('#name').removeClass('is-invalid');
                        $('#errorname').text('');
            ...
            $(document).on('click','.delete',function () {
                if (confirm('Bạn muốn xóa?')){
                    $.ajax({
                        url:'{{asset("admin/book/nxb/delete")}}',
                        type:'GET',
                        data:{id:$(this).val()},
                        success:function (data) {
                            alert(data.success);
            ...
            $('#formsubmit').submit(function (e) {
                ...
                if($('#action').val()==='Add'){
                    ...
                    var _token=$('input[name="_token"]').val();
                    $.ajax({
                        url:'{{asset('admin/book/nxb/update')}}',
                        type:'POST',
                        dataType:'json',
